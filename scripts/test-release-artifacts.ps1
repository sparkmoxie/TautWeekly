...
foreach ($archiveName in $expected.Keys) {
    ...
    try {
        ...
        Assert-True ($renderer.Contains('$tvShows = @($Stats.TvShowItems | Select-Object -First 4)')) "$archiveName does not enrich watched TV shows for IMDb ratings."
        Assert-True ($renderer.Contains('Get-OptionalStringProperty -InputObject $item -Name "DesignImdbRating"')) "$archiveName does not render IMDb ratings in watched-TV statistics."
        Assert-True ($renderer.Contains('function Get-PlexWatchRatings')) "$archiveName lacks the exact-slug public Plex rating fallback."
        Assert-True ($renderer.Contains('"User-Agent"      = "TautWeekly-for-Plex/0.8.0"')) "$archiveName does not identify the tokenless public Plex rating fallback."
        Assert-True ($renderer.Contains('"Accept-Language" = "en-US,en;q=0.9"')) "$archiveName does not request stable provider-labelled rating text."
