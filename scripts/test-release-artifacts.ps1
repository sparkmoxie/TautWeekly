...
function Assert-RendererContract([string]$PackageName, [string]$Renderer) {
    ...
    Assert-True ($Renderer.Contains('"User-Agent"      = "TautWeekly-for-Plex/0.8.2"')) "$PackageName does not identify the tokenless public Plex rating fallback."
    Assert-True ($Renderer.Contains('"X-Plex-Version"           = "0.8.2"')) "$PackageName does not identify the authenticated hosted metadata fallback."
