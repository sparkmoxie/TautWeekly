...
or renderer. The PowerShell fixture accepts only the fixed `PreviewAll` and
`SendTestAll` Manager contracts, requires the `FixtureOnly` marker, and waits
...
SMTP acceptances without opening a connection. Both write one sanitized
structured result. The fixture contains no network, SMTP, browser-launch, or
scheduler operation.
