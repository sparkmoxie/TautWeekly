...
func TestDiagnosticHistoryIsBoundedSanitizedAndPrivate(t *testing.T) {
	...
	if history.MaximumEntries != diagnosticHistoryLimit || history.RetentionDays != 30 {
...
func TestDiagnosticHistoryPrunesExpiredAndMalformedEvents(t *testing.T) {
	data := t.TempDir()
	now := time.Date(2031, 4, 18, 16, 30, 0, 0, time.UTC)
	path := filepath.Join(data, "diagnostic-history.jsonl")
	expired := DiagnosticEvent{
		SchemaVersion: diagnosticSchemaVersion,
		RecordedAtUTC: now.Add(-diagnosticHistoryAge - time.Hour).Format(time.RFC3339),
		Area:          "configuration",
		Outcome:       "passed",
		Code:          "config-saved",
	...
	history := store.History()
	if len(history.Events) != 1 || history.Events[0].Code != "smtp-result-failed" {
