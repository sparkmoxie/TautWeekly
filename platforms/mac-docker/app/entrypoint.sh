...
if ! getent group "$PGID" >/dev/null 2>&1; then
  groupmod -o -g "$PGID" plexweekly
else
  existing_group="$(getent group "$PGID" | cut -d: -f1)"
  usermod -g "$existing_group" plexweekly
fi
usermod -o -u "$PUID" plexweekly

mkdir -p /data/assets /data/logs /data/output/posters /data/output/media
cp -an /opt/plexweekly/assets-default/. /data/assets/ 2>/dev/null || true
...
if [[ ! -f /data/config.example.json ]]; then
  cp /opt/plexweekly/config.example.json /data/config.example.json
fi
chown -R "$PUID:$PGID" /data
chmod 700 /data 2>/dev/null || true
[[ ! -f /data/config.json ]] || chmod 600 /data/config.json 2>/dev/null || true

exec gosu "$PUID:$PGID" /opt/plexweekly/run-service.sh
