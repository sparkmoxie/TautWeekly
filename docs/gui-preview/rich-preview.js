...
(() => {
  ...
  const styles = `
    ...
    @media(max-width:560px){.stats-media-stack,.stats-summary-grid{margin-inline:12px}.stats-media-card .watched-list,.stats-summary-grid{grid-template-columns:1fr}.metric.compact{min-height:164px}}
  `;
  ...
  function statsMediaCard(items, kind) {
    ...
    return `<article class="stats-media-card"><div class="stats-heading"><img src="${media(icon)}" alt=""><div><div class="stats-heading-count">${items.length}</div><div class="stats-heading-label">${label}</div></div></div><div class="watched-list">${watchedRows(items, kind)}</div></article>`;
  }
