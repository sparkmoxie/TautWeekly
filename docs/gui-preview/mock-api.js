...
(() => {
  ...
  const editorFields = [
    ...
    field("PlexWebUrl", "Open Plex button URL", "Identity", "url", "https://app.plex.tv/desktop/", { required: true }),
    ...
    field("FromEmail", "From email", "Email", "email", "newsletter@starlight.invalid", { required: true }),
    field("ReplyToEmail", "Reply-to email", "Email", "email", "hello@starlight.invalid"),
    field("TestEmail", "Test recipient", "Email", "operator@starlight.invalid", { required: true }),
    ...
    field("SmtpUsername", "SMTP username", "SMTP", "text", "newsletter@starlight.invalid"),
  ...
  const setupStatus = {
    ...
    updatedAtUtc: now(),
    steps: {
      choices: { state: "passed", summary: "3 active libraries and 18 fictional users loaded in memory." },
  ...
  function status() {
    ...
    return {
      ...
      previewSummary: `${previews.length} fictional previews ready`,
    };
  }
